...
class DynamicAgent:
    ...
    def __init__(self, all_data, gaussian_window=120, save_mat=None, normalize=False, start_idx=0):
        # os.environ['CUDA_LAUNCH_BLOCKING'] = '1'
        self.retrain = False
        self.tail_index = start_idx
        # print('start index is', self.tail_index)
        ...
        self.all_data[:, 1:] = torch.nn.functional.normalize(self.all_data[:, 1:], dim=0) * 300 if normalize \
            else self.all_data[:, 1:] * 30
        self.all_data[:, 0] = self.all_data[:, 0] * 30
        ...
        self.res_mean_mat = None
        self.res_upper_mat = None
        self.res_lower_mat = None
        self.run()  # runrunrun
        self.save()
    ...
    def run(self):
        ...
        while self.header_index < self.all_data.shape[0] - 1:
            gap += 1
            self.update_data()
            # if self.tail_index % 200 == 1:
            #     self.save()
    ...
    def fit(self):
        ...
        if not torch.all((self.permutation.matmul(self.permutation.T)).eq(torch.eye(14).to(self.device))):
            ...
            assert not torch.isnan(torch.sum(self.predication_data)), 'predication contains nan number at {0}'.format(
                self.header_index)
        ...
        assert not torch.isnan(torch.sum(self.handled_data)), 'handled_data contains nan number at {0}'.format(
            self.header_index)
    ...
    def fit_conflict_points(self):
        ...
        for conflict_point_idx in conflict_points:
            ...
            for nebulous_idx in nebulous_idxes:
                ...
                # print('handled data is', self.handled_data[nebulous_idx, :])
                cdist = torch.cdist(self.handled_data[col, :].view(1, -1), self.last_data[nebulous_idx, :].view(1, -1))
                if cdist < distance:
                    row = nebulous_idx
                    distance = cdist
                print('distance is', distance)
    ...
    def fit_lost(self):
        ...
        for idx, lost_label_idx in enumerate(lost_labels):
            ...
            pred_distance = torch.cdist(self.handled_data[col, :].view(1, -1),
                                        self.predication_data[col, :].view(1, -1))
    ...
    def backward_interpolation(self, index):
        ...
        assert self.lost_end_idx[index] > self.lost_start_idx[
            index], f"lost end index is not greater than lost start index at {index}"
    ...
    def save(self):
        ...
        self.res_mean_mat = res_mean_mat if self.res_mean_mat is None else torch.cat(
            (self.res_mean_mat, res_mean_mat), dim=0)
        self.res_upper_mat = res_upper_mat if self.res_upper_mat is None else torch.cat(
            (self.res_upper_mat, res_upper_mat), dim=0)
        self.res_lower_mat = res_lower_mat if self.res_lower_mat is None else torch.cat(
            (self.res_lower_mat, res_lower_mat), dim=0)
        m_dic = {
            'time': self.valid_times.cpu().numpy() / 30,
            'mean': self.res_mean_mat.cpu().numpy() / 30,
            'upper': self.res_upper_mat.cpu().numpy() / 30,
            'lower': self.res_lower_mat.cpu().numpy() / 30,
            'all_data': self.all_data.cpu().numpy() / 30
        }
