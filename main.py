...
def main():
    ...
    labeler.optimize()
    # time value should be Nx1 shape
    test_time = tensor_data[gaussian_window, 0].view(-1)
...
def test():
    ...
    all_data = ridl.load_mat_file(file_path)['imputed_data']
    tensor_data = torch.tensor(all_data, dtype=torch.float64).to(device)
    agent = dynamicagent.DynamicAgent(tensor_data, save_mat='result.mat', gaussian_window=120, normalize=False, start_idx=20000)
...
def validate():
    ...
    labeled_file_path = data_dir + label_data + '/' + label_prefix + label_data + label_time + label_suffix
    unlabeled_file_path = data_dir + label_data + '/' + label_prefix + label_data + label_time + unlabel_suffix
    # all_data = ridl.load_csv_data(file_path, mformat_header=False).to_numpy() * 60
    labeled_data = ridl.load_mat_file(labeled_file_path)['imputed_data']
    unlabeled_data = ridl.load_csv_data(unlabeled_file_path, mformat_header=False, header=0).to_numpy()
    # print('unlabeled data:', unlabeled_data[:2, :3])

    all_data = np.vstack((labeled_data, unlabeled_data))
    tensor_data = torch.tensor(all_data, dtype=torch.float64).to(device)
    agent = dynamicagent.DynamicAgent(tensor_data, save_mat='result01.mat', gaussian_window=120, normalize=False, start_idx=0)
